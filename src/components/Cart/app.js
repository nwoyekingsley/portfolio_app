...
class ProductDetail extends Component {
  ...
  total = (value, discountPrice) => {
    return Number(value * discountPrice);
  };
    ...
    return (
      <section className="ftco-section ftco-cart">
        <div className="container">
                      ...
                      {addedCart.map(addedData => {
                        return (
                          <tr key={addedData.id} className="text-center">
                            ...
                            <td className="price">{addedData.discountPrice}</td>
                            ...
                            <td className="total">
                              ${this.total(value, addedData.discountPrice)}
                            </td>
                          </tr>
                        );
                      })}
          ...
          {addedCart.map(addedData => {
            return (
              <div key={addedData.id} className="row justify-content-center">
                <div className="col col-lg-5 col-md-6 mt-5 cart-wrap ftco-animate">
                  <div className="cart-total mb-3">
                    <h3>Cart Totals</h3>
                    <p className="d-flex">
                      <span>Subtotal</span>
                      <span>{addedData.discountPrice}</span>
                    </p>
                    <p className="d-flex">
                      <span>Delivery</span>
                      <span>$0.00</span>
                    ...
                    <p className="d-flex">
                      <span>Discount</span>
                      <span>${addedData.realPrice}</span>
                    </p>
                    <hr />
                    <p className="d-flex total-price">
                      <span>Total</span>
                      <span>${this.total(value, addedData.discountPrice)}</span>
                    </p>
                  </div>
